...
export function addMessage(content, isUser = false) {
    ...
    if (!normalizedContent.trim()) {
        const fallbackText = isUser ? 'Pesan terkirim' : 'Balasan kosong';
        return addMessage(fallbackText, isUser);
    }

    if (isPersonaMessage(normalizedContent)) return;
    ...
    const contentParts = detectCodeBlocks(normalizedContent);
